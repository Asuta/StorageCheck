...
- 新目标默认使用“极速模式（逻辑大小）”，扫描更快；也可以切回“精确模式（实际占用）”。
...
3. 大多数情况下建议保留 `极速模式（逻辑大小）`，先把扫描频率和响应速度做上去。
4. 如果你要做一次非常细的文件级排查，或者特别在意压缩 / 稀疏 / 云占位文件的“实际占用”，可以临时切到 `精确模式（实际占用）`。
5. 先手动扫一次，生成第一份快照。
6. 后面就可以在右侧切换不同时间点的扫描结果，对比目录变化。
...
- `极速模式（逻辑大小）`：直接使用文件逻辑大小，扫描明显更快，适合长期定时记录。
- `精确模式（实际占用）`：会额外读取文件在磁盘上的实际占用，更适合排查压缩文件、稀疏文件、OneDrive 占位文件这类场景。
- 如果你主要关心“哪个目录最近变大了”，优先用 `极速模式` 就够了。

## 注意事项

- 某些系统保护目录会因为权限不足而跳过，扫描不会中断。
- `完整保存` 在大盘上会写入很多记录，明显比按层保存更重。
- `精确模式` 因为会额外查询每个文件的实际占用，速度会慢于 `极速模式`。
